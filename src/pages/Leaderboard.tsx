...
const Leaderboard = () => {
  const { data: leaders = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      (likesRes.data || []).forEach((l) => { likeMap[l.wallpaper_id] = (likeMap[l.wallpaper_id] || 0) + 1; });
      ...
      wallpapers.forEach((w) => {
        ...
        if (ratingMap[w.id]) { s.ratingSum += ratingMap[w.id].sum; s.ratingCount += ratingMap[w.id].count; }
      });
      ...
      return Object.entries(userStats)
        .map(([uid, s]) => {
          ...
          return {
            user_id: uid, username: profile?.username || "Unknown", display_name: profile?.display_name,
            avatar_url: profile?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${uid}`,
            total_likes: s.likes, total_downloads: s.downloads, avg_rating: avgRating, upload_count: s.uploads,
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        title="Wallpaper Creator Leaderboard – Top Creators"
        description="See the top wallpaper creators on WallNova ranked by likes, downloads, and ratings. Join the community and climb the leaderboard."
      ...
      <div className="pt-24 pb-16 container mx-auto px-4 max-w-3xl">
        ...
        {isLoading ? (
          <div className="space-y-4">
            {[...Array(5)].map((_, i) => <div key={i} className="glass-card rounded-2xl p-5 animate-pulse h-20" />)}
          </div>
        ) : leaders.length === 0 ? (
          ...
          <div className="space-y-3">
            {leaders.map((entry, i) => (
              <motion.div key={entry.user_id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                className={`glass-card rounded-2xl p-4 md:p-5 flex items-center gap-4 border transition-all ${
                  i === 0 ? "border-yellow-400/30 bg-yellow-400/5" : i === 1 ? "border-gray-300/20 bg-gray-300/5" : i === 2 ? "border-amber-600/20 bg-amber-600/5" : "border-[hsl(var(--glass-border))]"
                }`}
              >
                <div className="w-8 flex items-center justify-center">{rankIcon(i)}</div>
                <img src={entry.avatar_url!} alt={`${entry.username} avatar`} className="w-11 h-11 rounded-xl bg-muted object-cover" loading="lazy" />
                <div className="flex-1 min-w-0">
                  <p className="font-display text-sm font-semibold text-foreground truncate">{entry.display_name || entry.username}</p>
                  <p className="text-xs text-muted-foreground">{entry.upload_count} wallpaper{entry.upload_count !== 1 ? "s" : ""}</p>
                </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-10 glass-card rounded-2xl p-5 text-center text-sm text-muted-foreground">
...
const StatPill = ({ icon, value }: { icon: React.ReactNode; value: string }) => (
  <span className="flex items-center gap-1 text-xs text-muted-foreground">{icon} {value}</span>
);
